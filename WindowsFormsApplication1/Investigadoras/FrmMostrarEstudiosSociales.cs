...
{
    public partial class FrmMostrarEstudiosSociales : Telerik.WinControls.UI.RadForm
    {
        private string _numeroChip;
        private int numeroFilas;

        public FrmMostrarEstudiosSociales()
        {
            InitializeComponent();
            try
            {
        ...
        private void FrmMostrarEstudiosSociales_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                numeroFilas = gvInformesSociales.Rows.Count;
        ...
        private void FrmMostrarEstudiosSociales_FormClosing(object sender, FormClosingEventArgs e)
        {
            timerVerificar.Stop();
        }
        #endregion
        ...
        {
            DbDataContext varLinq = new DbDataContext();
            var resultTemp = varLinq.mostrarEstudiosSocialesAInvestigadoras(
                                                                        identificacion,
                                                                        cadena,
                                                                        fechaInicio,
                                                                        fechaFinal,
            ...
            this.gvInformesSociales.DataSource = resultTemp;
        }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (gvInformesSociales.SelectedRows.Count != 0)
                {
                    int selectedRow = gvInformesSociales.CurrentRow.Index;
                    SaveFileDialog guardar = new SaveFileDialog();
                    guardar.FileName = gvInformesSociales.Rows[selectedRow].Cells["expediente"].Value.ToString();
                    ...
                    if (guardar.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        this.guardarArchivo(guardar.FileName, gvInformesSociales.Rows[selectedRow].Cells["expediente"].Value.ToString());
                    }
                }
            }
            catch (Exception excepcion)
        ...
        private void btnImprimir_Click(object sender, EventArgs e)
        {
            try
            {
                if (gvInformesSociales.SelectedRows.Count != 0)
                {
                    int selectedIndex = gvInformesSociales.CurrentRow.Index;
                    word.Application wordApp = new word.Application();
                    wordApp.Visible = true;
                    string filePath = this.obtenerArchivoTemp(gvInformesSociales.Rows[selectedIndex].Cells["expediente"].Value.ToString());
        ...
        private void btnAbrir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    int selectedIndex = gvInformesSociales.CurrentRow.Index;
                    numeroExpediente = gvInformesSociales.Rows[selectedIndex].Cells["expediente"].Value.ToString();
        ...
        private void timerVerificar_Tick(object sender, EventArgs e)
        {
            ...
            {
                DbDataContext varLinq = new DbDataContext();
                int nuevasFilas = varLinq.estudio_social.Count();
                if (this.numeroFilas < nuevasFilas)
                {
                    ...
                    daAlertaNuevosInformesSociales.ContentText = "<html><strong><span style=\"font-size: 9pt; color: #004080\">Usted tiene " +
                                                            (nuevasFilas - numeroFilas) + " nuevos Informes Sociales</span></strong></html>";
                    daAlertaNuevosInformesSociales.Show();
                    this.numeroFilas = nuevasFilas;
                }
            }
            catch (Exception excepcion)
            {
